...
type Invoice = {
  ...
  normalized?: { vendorName?: string | null; customerName?: string | null; customerEmail?: string | null; customerPhone?: string | null; invoiceNumber?: string | null; dueDate?: string | null; total?: Money }
...
// Derive follow-ups from real invoices: approved + unpaid + past due -> payment reminder,
// needs_review -> acknowledgement, missing customer/number -> data request.
function buildFollowUps(invoices: Invoice[]): FollowUp[] {
  const list: FollowUp[] = []
  for (const inv of invoices) {
    const customer = inv.normalized?.customerName || inv.normalized?.vendorName || inv.source?.originalName || "Unknown customer"
    const number = inv.normalized?.invoiceNumber || inv.id.slice(0, 8)
    const amount = inv.normalized?.total?.amount ?? null
    const overdue = invoicePaymentState(inv) === "overdue"

    if (overdue) {
      list.push({
        id: `${inv.id}-reminder`, customer, invoiceNumber: number, channel: inv.normalized?.customerPhone ? "sms" : "email", priority: "high", amount,
      ...
      list.push({
        id: `${inv.id}-ack`, customer, invoiceNumber: number, channel: inv.normalized?.customerEmail ? "email" : "sms", priority: "low", amount,
    ...
    if (!(inv.normalized?.customerName || inv.normalized?.vendorName) || !inv.normalized?.invoiceNumber) {
      if (["needs_review", "verified"].includes(inv.status)) {
        list.push({
          id: `${inv.id}-data`, customer, invoiceNumber: number, channel: inv.normalized?.customerPhone ? "sms" : "email", priority: "medium", amount,
...
export default function FollowUps() {
  ...
  return <FinancePageShell title="Follow-ups" description="Customer outreach through email and SMS, drafted from your invoices.">
    <div className="grid gap-4 sm:grid-cols-3">
      <StatCard label="Ready to send" value={`${followUps.length - sent.length}`} detail="Across email and SMS" color="text-[#86efac]" />
      <StatCard label="Email" value={`${counts.email}`} detail="Notices and payment reminders" />
      <StatCard label="SMS" value={`${counts.sms}`} detail="Notices and payment reminders" />
    </div>
            ...
            {visible.map((f) => {
              ...
              return (
                <tr key={f.id} className="border-t border-[#181818] align-top text-sm hover:bg-[#111]">
                  <td className="px-5 py-4"><p className="font-medium text-[#ddd]">{f.customer}</p><p className="mt-1 font-mono text-[10px] text-[#666]">{f.invoiceNumber}{f.amount != null ? ` · ${formatMoney(f.amount)}` : ""}</p></td>
