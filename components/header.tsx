...
export function Header() {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 flex h-20 items-center justify-between border-b border-white/[0.04] bg-black/70 px-4 backdrop-blur-2xl sm:px-6">
      <Link href="/" aria-label="Mercury Computers Limited dashboard" className="flex shrink-0 items-center rounded-lg bg-white p-1.5 shadow-sm">
        <MercuryLogo variant="mark" className="h-9 w-9 sm:hidden" priority />
        <MercuryLogo className="hidden h-9 w-[184px] sm:block" priority />
      </Link>
      <div className="flex items-center gap-2">
        <Link href="/invoices" className="hidden h-10 items-center gap-2 rounded-xl border border-[#242424] bg-[#101010] px-3 text-xs text-[#777] transition hover:border-[#333] hover:text-white lg:flex">
          <Icon name="SearchNormal1" size={16} />
          Search invoices
          <span className="ml-4 rounded border border-[#303030] px-1.5 py-0.5 text-[9px]">⌘ K</span>
        </Link>
        <ThemeToggle />
        <button className="relative grid h-10 w-10 place-items-center rounded-xl border border-[#242424] bg-[#101010] text-[#999] transition hover:text-white" aria-label="Notifications">
        ...
        <Link href="/review" className="hidden h-10 items-center gap-2 rounded-xl bg-[#86efac] px-4 text-xs font-semibold text-black transition hover:bg-[#a7f3c0] md:flex">
          <Icon name="MagicStar" size={16} variant="Bold" /> AI invoice review
        </Link>
      </div>
    </header>
  )
